...
FilterLayer FilterLayer::makeMovingAverage(std::size_t windowSize) {
    return FilterLayer(FilterType::MovingAverage, windowSize);
}

FilterLayer FilterLayer::makeLowPass(float alpha) {
    return FilterLayer(FilterType::LowPass, 5, alpha);
}