...
// Low-pass 3: every sensor has independent filter state.
{
    ...
    filter.process(
        {
            sample("temp-1",20.0f),
            sample("pressure-1",100.0f)
        }
    );

    auto result = filter.process(
        {
            sample("temp-1",30.0f),
            sample("pressure-1",200.0f)
        }
    );

    nearlyEqual(result[0].value,(0.5*20 + 0.5*30),"Temp must use its own low-pass state");
    nearlyEqual(result[1].value,(0.5*100 + 0.5*200),"Pressure must use its own low-pass state");
}
...

{
    ...
    auto invalid = filter.process({
        sample("temp-1",999.0f,SensorStatus::Invalid)
    });
    ...
    nearlyEqual(recovered[0].value,40.0f,"First valid sample after invalid data must start fresh");
...
// Low-pass 5: disconnect also resets state, and reconnection starts fresh
{
    ...
    auto disconnected = filter.process({
        sample("temp-1", 0.0f, SensorStatus::Disconnected)
    });
