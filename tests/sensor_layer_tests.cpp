...
int main()
{
    ...
    //3. Multiple sensors retain their own data and order
    {
        ...
        layer.addSensor(std::make_unique<FakeSensor>(
        makeSample("flow-1", SensorType::Flow, 2.5f, "L/min")
    ));
        ...
        nearlyEqual(samples[0].value, 25.0f,
                    "Temperature value must be preserved");

        assert(samples[1].id == "pressure-1");
        assert(samples[1].type == SensorType::Pressure);
        nearlyEqual(samples[1].value, 101.3f,
                    "Pressure value must be preserved");

        assert(samples[2].id == "flow-1");
        assert(samples[2].type == SensorType::Flow);
        nearlyEqual(samples[2].value, 2.5f,
                    "Flow value must be preserved");
...
// 4. A disconnected sensor becomes invalid
    ...
    layer.addSensor(std::make_unique<FakeSensor>(
        makeSample("temp-1", SensorType::Temperature,
                   123.0f, "C", SensorStatus::Disconnected)
    ));
    ...
    nearlyEqual(samples[0].value, 123.0f,
                "SensorLayer must preserve raw disconnected value");
...
// 5.NaN becomes invalid
{
    ...
    assert(samples.size() == 1);
    assert(samples[0].status == SensorStatus::Invalid &&
           "NaN measurement must be invalid");
}
